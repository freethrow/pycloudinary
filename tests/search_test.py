...
TEST_IMAGE = test_helper.TEST_IMAGE
TEST_TAG = test_helper.TEST_TAG
UNIQUE_TAG = test_helper.UNIQUE_TAG
SUFFIX = test_helper.SUFFIX
...
class SearchTest(unittest.TestCase):
    ...
    def test_should_add_max_results_as_dict(self):
        ...
        query = Search().max_results('format:jpg').as_dict()
        self.assertEqual(query, {"max_results": 'format:jpg'})

    @unittest.skipUnless(cloudinary.config().api_secret, "requires api_key/api_secret")
    def test_should_add_next_cursor_as_dict(self):

        query = Search().next_cursor('format:jpg').as_dict()
        self.assertEqual(query, {"next_cursor": 'format:jpg'})
    ...
    def test_should_paginate_resources_limited_by_tag_and_ordered_by_ascending_public_id(self):
        ...
        for i in range(0, 3): # get one resource at a time
            results = Search().max_results(1).expression("tags:{0}".format(UNIQUE_TAG)).sort_by('public_id', 'asc').next_cursor(results['next_cursor']).execute()
            self.assertEqual(len(results['resources']), 1)
            self.assertEqual(results['resources'][0]['public_id'], public_ids[i], 
                             "{0} found public_id {1} instead of {2} ".format(i, results['resources'][0]['public_id'], public_ids[i]))
